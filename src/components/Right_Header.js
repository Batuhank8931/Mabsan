  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (
        ...
        line.classList.add('animate-line-right');
      } else {
        // If the line is not visible, remove the class
        line.classList.remove('animate-line-right');
      }
    };
  ...
  return (
    <div className="baslik p-md-5 p-3 ">
      <div className="d-flex justify-content-between">
        <div className="left-div d-flex align-items-center w-8">
          <img src={polygon} alt="Polygon" />
        </div>
        <div className="middle-div d-flex align-items-center">
        <div ref={lineRef} className="line animate-line-right"></div> {/* Use a ref to select the line element */}
        </div>
        <div className="right-div d-flex align-items-center ml-5">
          <label className="line_header">{sectorLabel}</label> {/* Use the prop value here */}
        </div>
      </div>
    </div>
  );
